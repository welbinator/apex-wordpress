...
class Themify_Updater_Notifications {
    ...
    private function wrapper ( $notifications, $type ) {
        ...
        if ( sizeof($notifications, 0) > $this->notification_group && $type === 'warning') {
            $str = '';
            $wrapper = '<div class="notifications"><div class="%s themify-updater notification-group"><span><strong>'. __('Themify\'s', 'themify-updater') .' '. count($notifications) .' '. __('updates are available', 'themify-updater') .'</strong></span>%s</div></div>';
