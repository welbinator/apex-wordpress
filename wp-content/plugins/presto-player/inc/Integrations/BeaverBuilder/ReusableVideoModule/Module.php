...
class Module extends \FLBuilderModule
{
    ...
    {
        parent::__construct([
            'name'            => __('Presto Player', 'presto-player'),
            'description'     => __('Presto Player Video', 'presto-player'),
                ...
                'sections' => [
                    'video_select' => [
                        ...
                        'fields' => [
                            'video_select_ajax' => [
                                'type' => 'raw',
                                'label' => __('Select A Video', 'fl-builder'),
            ...
            <div class="fl-builder--category-select" x-data="window.prestoBBDropdown({nonce: '<?php echo wp_create_nonce('presto_bb_nonce'); ?>'})" x-init="init">
                <div class="fl-builder--selector-display" x-on:click="open">
                    <button class="fl-builder--selector-display-label">
                        <span class="fl-builder--group-label">Video</span>
                        <span class="fl-builder--current-view-name" x-text="video.name || 'Select a video'"></span>
                    </button>
                </div>

                <div class="presto-builder--selector-menu">
                    <div class="presto-builder--menu" x-show="isOpen()" x-on:click.away="close">
                        <input class="presto-builder--dropdown-search" x-ref="searchbox" type="text" x-model="search" placeholder="<?php _e('Search for a video', 'presto-player'); ?>" />
